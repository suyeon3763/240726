...
def existential_quiz():
    ...
    philosophers = {"키르케고르": 0, "하이데거": 0, "야스퍼스": 0, "사르트르": 0}
    ...
    # 각 질문에 대한 응답을 받고 점수를 계산합니다.
    for i, question in enumerate(questions):
        answer = st.radio(question, ["매우 그렇다", "그렇다", "보통이다", "그렇지 않다", "전혀 그렇지 않다"])
        philosopher = responses[str(i + 1)][answer]
        philosophers[philosopher] += scoring[answer]
    
    # 결과 도출
    highest_score = max(philosophers, key=philosophers.get)
